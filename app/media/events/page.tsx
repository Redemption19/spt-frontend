...
export default function EventsPage() {
  ...
  // Fetch events from backend - fetch both published and completed events
  const { events: allEvents, loading, error, refetch } = useEvents();

  // Transform and filter events - only show published and completed events on frontend
  ...
  const filteredUpcomingEvents = useMemo(() => {
    return transformedEvents.filter(event => {
      ...
      const matchesType = eventTypeFilter === '' || eventTypeFilter === 'all' || event.type === eventTypeFilter;
      const matchesRegion = regionFilter === '' || regionFilter === 'all' || event.region === regionFilter;

      return isUpcoming && matchesSearch && matchesType && matchesRegion;
    });
  }, [transformedEvents, searchTerm, eventTypeFilter, regionFilter]);
